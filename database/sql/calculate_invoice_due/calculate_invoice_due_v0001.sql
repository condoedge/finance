...
BEGIN
    ...
    SELECT SUM(-IFNULL(ip.payment_applied_amount, 0) + IFNULL(in_d.extended_price, 0)) INTO invoice_due FROM fin_invoices as i
        left join fin_invoice_details as in_d on i.id = in_d.invoice_id
        left join fin_invoice_payments as ip on i.id = ip.invoice_id
