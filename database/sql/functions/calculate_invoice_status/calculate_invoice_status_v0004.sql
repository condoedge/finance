...
BEGIN
    ...
    -- A credit note is owed BY us, never TO us, so it can never fall overdue.
    -- Guarded on the type's sign so historical rows carrying a due date are covered too.
    SELECT it.sign_multiplier INTO sign_multiplier
    FROM fin_invoices i
    JOIN fin_invoice_types it ON i.invoice_type_id = it.id
    WHERE i.id = p_invoice_id;

    IF sign_multiplier < 0 THEN
        RETURN FALSE;
    END IF;
...
BEGIN
    ...
    DECLARE p_voided_at DATETIME DEFAULT NULL;

    -- A voided invoice is cancelled, whatever its balance says. Checked first because a
    -- void settles the balance to zero, which would otherwise read as 'paid'.
    SELECT voided_at INTO p_voided_at FROM fin_invoices WHERE id = p_invoice_id;

    IF p_voided_at IS NOT NULL THEN
