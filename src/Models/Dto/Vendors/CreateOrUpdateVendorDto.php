...
class CreateOrUpdateVendorDto extends ValidatedDTO
{
    use EmptyDefaults;

    /**
     * Send this field as null to create a new vendor.
     * @var int
     */
    public ?int $id;
    ...
    public function rules(): array
    {
        return [
             ...
             * @example null
             */
            'id' => ['nullable', 'integer', 'exists:fin_vendors,id'],
    ...
    public function after($validator): void
    {
        if (!$validator->errors()->has('address') && $validator->errors()->has('address*')) {
            $validator->errors()->add('address', __('translate.validation.vendor.the-address-must-be-complete'));
        }
    }
}
